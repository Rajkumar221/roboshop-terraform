...
module "rds" {
  source = "git::https://github.com/Rajkumar221/tf-module-rds.git"
  for_each = var.rds
  component = each.value["component"]
...
module "documentdb" {
  source = "git::https://github.com/Rajkumar221/tf-module-documentdb.git"
...
module "elasticcache" {
  source = "git::https://github.com/Rajkumar221/tf-module-elasticcache.git"
  for_each = var.elasticcache
  component = each.value["component"]
  engine = each.value["engine"]
  engine_version = each.value["engine_version"]
  replica_for_node_group = each.value["replica_for_node_group"]
...
module "eks" {
  source = "git::https://github.com/Rajkumar221/tf-module-eks.git"
  for_each = var.eks
